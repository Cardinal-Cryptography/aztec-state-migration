...
async function main() {
    userLog('Starting Aztec Node with read-only L1 time sync (TestDateProvider)...');
    ...
    if (!nodeConfig.l1Contracts.registryAddress || nodeConfig.l1Contracts.registryAddress.isZero()) {
        throw new Error('L1 registry address is required (set REGISTRY_CONTRACT_ADDRESS)');
    }
    ...
    nodeConfig.l1Contracts = { ...addresses, slashFactoryAddress: nodeConfig.l1Contracts.slashFactoryAddress };
    Object.assign(nodeConfig, config);

    // Sequencer setup: use validator key as publisher key if not set
    if (
        (!nodeConfig.publisherPrivateKeys || !nodeConfig.publisherPrivateKeys.length) &&
    ...
    if (nodeConfig.p2pEnabled && nodeConfig.bootstrapNodes && typeof nodeConfig.bootstrapNodes === 'string') {
