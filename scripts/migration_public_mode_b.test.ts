...
async function main() {
  ...
  const initTx = await oldApp.methods
    .init_some_struct({ a: TEST_A, b: TEST_B })
    .send({ from: oldDeployerManager.address })
    .wait();
  console.log(`   Init tx: ${initTx.txHash}`);
  console.log(`   SomeStruct = { a: ${TEST_A}, b: ${TEST_B} }\n`);
  ...
  // ============================================================
  // Derive account keys (same as Mode B private)
  // ============================================================
  console.log("6. Deriving account keys...");
  ...
  // ============================================================
  console.log("7. Building key note proofs...");
  ...
  const [keyNoteProof] = await oldUserWallet.buildNoteAndNullifierProofs(
    provenBlockNumber,
    [keyNotes[0]],
  ...
  console.log(`Migrated struct: { a: ${TEST_A}, b: ${TEST_B} }`);
}
